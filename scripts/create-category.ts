...
// Load categories configuration from JSON
function loadCategoriesConfig(): Record<string, CategoryConfig> {
...
function createCategoryProject(category: string, outputDir: string): void {
  ...
  categoryInfo.contracts.forEach(
    ({ path: contractPath, test: testPath, fixture, additionalFiles }) => {
      // Copy contract
...
function main(): void {
  ...
  const outputDir =
    args[1] || path.join(process.cwd(), "output", `fhevm-examples-${category}`);
